...
現在點數:${n[1]}`),t=`${$("#home").data("uri")}`,fetch(t,{method:"GET"}).then(()=>{sessionStorage.setItem("userPointsEdited","true")}).catch(n=>{alert(n)}));i.show();$(this).hide()}).catch(n=>{alert(n)})});r=$("#game").data("src");u=$("#game").data("uri");$("#game").index()!=-1?t=setInterval(function(){f()},4100):clearInterval(t)});